...
if(count($section_pages)) 
{
	...
	foreach ($contPages  as $contVal) 						
	{
		...
		if($cont_section == 6){
            ...
            $evdateb    = $event_date;
			$evdatec    = date('Ymd',$event_date);
            
            if($evdateb < $cont_now) { $event_past = '<span class="txtgreen txt10">[ARCHIVE]</span> &nbsp; '; }
		    if($evdatec == $cont_leo) { $event_past = '<span class="txtred txt10">[TODAY]</span> &nbsp; '; }
            
			//displayArray($itemArr['event_dates']);
...
-------------------------------------------------------------------------------------------------------*/ 
		...
		if($loopNum <= 4)	
		{
			...
			$item_Content .=  '<li class="row marg15_b padd10_b	"> <div class="subcolumns project_itemX" ><div class="project_wrapX">				
				...
				<div class="project_padd" style="min-height:70px;line-height:1.4">'. $image_disp .'  <a '.$item_link.'  data-id="'.$cont_id.'" class="linkCont bold block txt14">'.$event_past.' '.$cont_title.'</a>'.$title_date.''. $sector .' '. $cont_brief_plain .' </div>
				</div>
				</div> </div>
			</li>'; 
		}
...
<div class="panel panel-default panel-home-guts">
    <div class="panel-heading clearfix txtleft bg-white">
       <div class="col-md-8 nopadd"><h4 class=""><i class="fa fa-files-o"></i> &nbsp; Latest News & Events</h4></div>
