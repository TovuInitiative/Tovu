...
ORDER BY `mrfc_forum_categories`.`cat_current` DESC limit 0, 1;";
...
else
{
	...
	else
	{
		...
		$item_Content .= '<li> <div class="project_item" ><div class="project_wrap">
			<div class="project_name"><a href="'.$forum_page.'/?fc=category&id=' . $row['cat_id'] . '" class="txtwhite">' . $cat_name . '</a></div>
...
ORDER BY `mrfc_forum_topics`.`topic_date` DESC, `mrfc_forum_posts`.`post_date` DESC LIMIT 1;";
					...
					else
					{
						...
						if(mysqli_num_rows($topicsresult) > 0)
						{
							...
							$item_Content .= '<span class="txt11"> Last post &raquo; </span></span> <a class="txtwhite" href="'.$forum_page.'/?fc=topic&id=' . $topicrow['topic_id'] . '#' . $topicrow['post_id'] . '">' . $post_content . '</a> '  
...
<div class="box-contX">
	...
	<?php //echo display_PageTitle('Community Forums', 'h4', 'txtgray box-cont-title'); ?>
